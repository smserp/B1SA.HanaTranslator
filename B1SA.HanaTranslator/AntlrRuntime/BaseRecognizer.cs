...
namespace Antlr.Runtime
{
    ...
    using IDebugEventListener = IDebugEventListener;
    ...
    public abstract class BaseRecognizer
    {
        ...
        public virtual void Reset()
        {
            ...
            for (var i = 0; state.ruleMemo != null && i < state.ruleMemo.Length; i++) { // wipe cache
        ...
        {
            var prefix = SourceName ?? string.Empty;
            if (prefix.Length > 0)
                prefix += ' ';
        ...
        {
            var stopIndex = GetRuleMemoization(ruleIndex, input.Index);
        ...
        public virtual void Memoize(IIntStream input,
                            ...
                            int ruleStartIndex)
        {
            var stopTokenIndex = state.failed ? MemoRuleFailed : input.Index - 1;
        ...
        {
            var n = 0;
            for (var i = 0; state.ruleMemo != null && i < state.ruleMemo.Length; i++) {
